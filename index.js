...
const fire = () => {
  ...
  interval = setInterval(() => {
    ...
    laserPosition -= 15
    //before we do anthing, check we´re still in the grid
    if (laserPosition < 0) {
    ...
    //cleck interval, remove alien image, remove alien from postion, set a timeout for a boom emoji
...
const play = () => {
  ...
  let direction = 'right'
  //set interval to repeat update alien positions and drawing them
  interval = setInterval(() => {
    let movement 
    //if right
    if (direction === 'right') {
      if (atSide('right')) {
        //go down a row and reverse directions to the left
        movement = 15 - 1
        direction = 'left'
      } else {
        //continue right
